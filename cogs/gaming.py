...
def load_data_sync():
    """Synchronous load for the gaming cog."""
...
def save_data_sync(data):
    """Synchronous save for the gaming cog."""
    try:
...
class GamingCog(commands.Cog):
    ...
    async def game_match(
        ...
        embed = discord.Embed(
            title="\U0001f3ae NEW MATCH STARTED \U0001f3ae",
            description=(
                f"**{interaction.user.mention}** vs **{opponent.mention}**\n\n"
                f"\U0001f3b2 Game: **{game}**\n"
    ...
    async def game_result(
        ...
        embed = discord.Embed(
            title="\U0001f3c6 MATCH RESULT \U0001f3c6",
            description=(
                f"**{winner.mention}** defeated **{loser.mention}**!\n\n"
                f"\U0001f4ca **{winner.display_name}**: {w_wins}W / {data['users'][str(winner.id)]['gaming_losses']}L\n"
                f"\U0001f4ca **{loser.display_name}**: {l_wins}W / {data['users'][str(loser.id)]['gaming_losses']}L"
            ),
            color=0x57F287,
        )
        await interaction.response.send_message(embed=embed)
    ...
    async def gaming_stats(
        ...
        embed = discord.Embed(
            title=f"\U0001f3ae {target.display_name}'s Gaming Stats",
            color=0x5865F2,
        )
        embed.add_field(name="\U0001f3c6 Wins", value=str(wins), inline=True)
        embed.add_field(name="\U0001f480 Losses", value=str(losses), inline=True)
        embed.add_field(name="\U0001f4ca Win Rate", value=f"{winrate:.0f}%", inline=True)
    ...
    async def on_voice_state_update(
        ...
        # Only trigger when someone JOINS a game channel
            ...
            if not before.channel or before.channel.id not in GAME_CHANNELS:
                ...
                if text_channel:
                    ...
                    await text_channel.send(
                        f"\U0001f440 Yo {member.mention}, hopping into **{game_name}**? "
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(url, headers=headers) as resp:
                    ...
                    if channel:
                        embed = discord.Embed(
                            title="\U0001f916 AUTO-RESOLVED CHESS MATCH \U0001f916",
                            description=(
                                f"{winner_discord} destroyed {loser_discord} on Lichess!\n\n"
                                f"\u26a1 Speed: **{speed.capitalize()}**\n"
                                f"\U0001f194 Game: `{game_id}`"
                            ),
                            color=0x57F287,
                        )
