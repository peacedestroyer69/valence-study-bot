...
def _get_model_instance(key: str, model_name: str):
    ...
    if cache_key not in _cached_models:
        try:
            # Import client from google.generativeai explicitly to bind client to model instance
            from google.generativeai import client as genai_client
            genai.configure(api_key=key)
            model = genai.GenerativeModel(model_name)
            # Force resolve and cache the client to ensure thread-safety and decouple from global configuration
