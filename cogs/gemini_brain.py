...
try:
    ...
    try:
        ...
        logging.warning("[GEMINI] google-genai or google-generativeai not installed.")
...
def _clean_json_str(raw_text: str) -> dict:
    """Safely cleans markdown code blocks and escapes single LaTeX backslashes for valid JSON parsing."""
    if not raw_text:
        return None
    # Fix control characters generated by backslash f (\frac -> \x0c)
    cleaned = raw_text.replace('\x0c', '\\f').replace('\x07', '\\a').replace('\x08', '\\b').replace('\x0b', '\\v')
    ...
    # Pass 2: Escape unescaped backslashes
    cleaned_bs = re.sub(r'\\(?![/"bfnrtu])', r'\\\\', cleaned)
    try:
        return json.loads(cleaned_bs)
    except Exception:
        pass

    # Pass 3: Extract outer JSON object block
    ...
    if match:
        ...
        try:
            ...
            try:
                obj_bs = re.sub(r'\\(?![/"bfnrtu])', r'\\\\', obj_str)
                return json.loads(obj_bs)
            except Exception:
                pass
    return None
