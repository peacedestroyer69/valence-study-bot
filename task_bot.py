...
import base64
ypt_token = os.getenv("BOT_TOKEN")
YPT_BOT_ID = 1517449638091689985
if ypt_token:
    try:
        first_segment = ypt_token.split(".")[0]
        padding_needed = len(first_segment) % 4
        if padding_needed:
            first_segment += "=" * (4 - padding_needed)
        decoded_bytes = base64.b64decode(first_segment)
        YPT_BOT_ID = int(decoded_bytes.decode("utf-8"))
        logger.info(f"Dynamically decoded YPT_BOT_ID from BOT_TOKEN: {YPT_BOT_ID}")
    except Exception as e:
        logger.error(f"Failed to decode YPT_BOT_ID from BOT_TOKEN: {e}. Using default fallback: {YPT_BOT_ID}")
    ...
    if message.channel.id in MODERATED_CHANNELS:
        ...
        if message.author.id != YPT_BOT_ID:
            ...
            try:
                ...
                logger.info(f"Deleted message successfully. Sender: {author_info}, Channel: {channel_info}, Content: {content}")
            except Exception as e:
...
async def shutdown(sig=None):
    ...
    if not bot.is_closed():
        await bot.close()
        logger.info("Task bot client closed.")
        
    # 2. Clean up keep-alive runner
    if hasattr(bot, "keepalive_runner") and bot.keepalive_runner:
        try:
            await bot.keepalive_runner.cleanup()
            logger.info("Keep-alive runner cleaned up successfully.")
        except Exception as e:
            logger.error(f"Error cleaning up keep-alive runner: {e}")
