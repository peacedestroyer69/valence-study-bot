...
class ToggleSubtaskSelect(discord.ui.Select):
    def __init__(self, checklist, parent_view):
        ...
        super().__init__(
            placeholder="Select a subtask to toggle status...",
            options=options,
            min_values=1,
            max_values=1
        )
...
class TaskDetailView(discord.ui.View):
    ...
    def get_embed(self) -> discord.Embed:
        ...
        # Determine embed color based on task status, type, and priority (Premium Neon Scheme)
        if t.get("status") == "completed":
            color = 0x00FF66  # Neon Mint Green
        elif t.get("is_habit"):
            color = 0x8A2BE2  # Deep Violet
        else:
            color = {"High": 0xFF007F, "Medium": 0xFFB800, "Low": 0x00F0FF}.get(t.get("priority"), 0xFFB800)
    ...
    @discord.ui.button(label="⏱️ Start Focus (25m)", style=discord.ButtonStyle.blurple, row=1)
    ...
    @discord.ui.button(label="🗑️ Delete Task", style=discord.ButtonStyle.red, row=2)
...
class TaskPaginationView(discord.ui.View):
    ...
    def get_embed(self) -> discord.Embed:
        color = 0xFF007F if self.target_user.id == 856485470171299891 else 0x00F0FF
        embed = discord.Embed(
...
class DMSnoozeView(discord.ui.View):
    ...
    @discord.ui.button(label="⏱️ Snooze 1h", style=discord.ButtonStyle.blurple)
    ...
    @discord.ui.button(label="📅 Snooze 1d", style=discord.ButtonStyle.secondary)
...
class TasksCog(commands.Cog, name="Tasks"):
    ...
    def __init__(self, bot: commands.Bot):
        ...
        self.last_morning_agenda_dates = {}  # uid_str -> date_str
        self.last_evening_nudge_dates = {}   # uid_str -> date_str
        self.last_weekly_digest_dates = {}   # uid_str -> date_str
        ...
        try:
            ...
            if now.hour == 8:
                for uid_str in user_ids:
                    if self.last_morning_agenda_dates.get(uid_str) == today_str:
                        continue
                    try:
                        user_id = int(uid_str)
                        ...
                        self.last_morning_agenda_dates[uid_str] = today_str
            ...
            if now.hour == 21:
                for uid_str in user_ids:
                    if self.last_evening_nudge_dates.get(uid_str) == today_str:
                        continue
                    try:
                        profile = await asyncio.to_thread(task_db.get_user_profile, uid_str)
                        streak = profile.get("streak", 0)
                        last_completed = profile.get("last_completed_date")
                        today_str_db = task_db.get_ist_date_str()
                        
                        if streak > 0 and last_completed != today_str_db:
                            user_id = int(uid_str)
                            user = await self.bot.fetch_user(user_id)
                            if user:
                                embed = discord.Embed(
                                ...
                                await user.send(embed=embed)
                                self.last_evening_nudge_dates[uid_str] = today_str
            ...
            if now.hour == 20 and now.weekday() == 6:
                for uid_str in user_ids:
                    if self.last_weekly_digest_dates.get(uid_str) == today_str:
                        continue
                    try:
                        user_id = int(uid_str)
                        user = await self.bot.fetch_user(user_id)
                        ...
                        self.last_weekly_digest_dates[uid_str] = today_str
            ...
            logger.error(f"[PLANNER] Error in planner_alerts_task loop: {e}", exc_info=True)
