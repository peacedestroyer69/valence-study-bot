...
def _decay_streak_if_needed(profile: dict) -> bool:
    """Decays the user's completion streak to 0 if they missed the completion window.
    ...
    if last_completed != today_str and last_completed != yesterday_str:
        if profile.get("streak", 0) > 0:
            profile["streak"] = 0
            update_user_profile(profile["user_id"], {"streak": 0})
            return True
    return False
...
def get_user_profile(user_id: str) -> dict:
    ...
    with db_lock:
        ...
        _decay_streak_if_needed(profile)
        return profile
...
def get_leaderboard() -> list:
    ...
    with db_lock:
        ...
        try:
            cursor = conn.cursor()
            cursor.execute("SELECT user_id, xp, level, streak, last_completed_date, total_completed FROM users")
            rows = cursor.fetchall()
        finally:
            conn.close()
        ...
        for r in rows:
            u = {
                ...
                "level": r[2],
                "streak": r[3],
                "last_completed_date": r[4],
                "total_completed": r[5]
            }
            _decay_streak_if_needed(u)
            leaderboard.append(u)
            
        leaderboard.sort(key=lambda u: (u.get("level", 1), u.get("xp", 0)), reverse=True)
...
def reset_habit(task_id: str):
    """Resets a completed habit status back to pending and clears checklist item statuses."""
    with db_lock:
        task = get_task(task_id)
