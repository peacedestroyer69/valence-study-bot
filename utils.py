...
def apply_global_gateway_lockout(bot: commands.Bot, locked_role_id: int = 1534636469443100692):
    """
    Patches the bot's dispatch mechanism to drop any events (messages, interactions)
    from users who possess the specified locked_role_id, ensuring zero loopholes.
    """
    ...
    if member.voice and member.voice.channel:
        ...
        if "general" not in v_name and "lobby" not in v_name:
            try:
                ...
                logging.info(f"[QUARANTINE VC KICK] Disconnected {member.display_name} from #{member.voice.channel.name}")
